...
### 2026-04-13 - 修复管理员创建与越权管理逻辑漏洞
**目标**: 解决新建管理员角色数据库截断报错，并修复管理员权限“金钟罩”的系统设计逻辑漏洞。

**改动**:
1. **数据库层**：
   - 修改表结构，修复 role 字段由于 `ENUM` 类型导致插入 "LIBRARIAN" 被截断引发的 500 报错（从 `ENUM` 变回 `VARCHAR(20)`）。
2. **后端 (`UserServiceImpl.java`)**：
   - 修复 DTO `RegisterRequest` 使其正确接收前端发送的 role 参数。
   - 调整 `updateUserStatus` 和 `deleteUser` 控制逻辑：将“免死金牌”（不可禁用、不可删除）特权死死绑定在 `username = 'admin'` 的系统初始超级管理员身上。允许初始 `admin` 去删除/禁用其他后来创建的普通管理员。
3. **前端 (`admin.html`)**：
   - 修复新增管理员下拉列表组件（把下拉值修改回纯英文角色名如 `ADMIN`, `LIBRARIAN`）和提交函数，使得能够正确传递 role 参数。
   - 增加并导出 `canDelete` 权限判断函数，修改 `canToggleStatus` 函数。现在列表界面的删除与状态按钮显示受严格约束：只保护初始 `admin` 账号不可操作，彻底解决了之前管理员无法互相管理的问题。
